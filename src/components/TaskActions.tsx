...
export function TaskActions({ item, childrenOf, onOpen }: TaskActionsProps) {
  ...
  return (
    <span className="trow-actions" ref={ref} onClick={(e) => e.stopPropagation()}>
      ...
      {menu === 'more' && (
        /* Two refs, two jobs: one decides which way the menu opens, the other
           drives it from the arrow keys once it has. */
        <div
          className={`popover rowmenu${placement.className}`}
          role="menu"
